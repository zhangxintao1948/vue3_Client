      ...
      <form>
        <div :class="{on:loginType}">
          <section class="login_message">
            ...
            @click.prevent="getCode" :class="{right_phone_number:isRightPhone}">
            {{computTime>0?`已发送验证码，剩余${computTime}s`:'获取验证码'}}
            </button>
          </section>
...
import { reqCode,reqSmsLogin,reqPwdLogin } from '../../api'

  export default {
    data(){
      return {
        loginType:true,//短信登录为true 密码登录为false
        phone:'',
        code:'',//短信验证码
        name:'',//用户名
        pwd:'',//密码
        captcha:'',//图片验证码
        computTime:0,//计算剩余时间，为0不计时
        isShowPwd:false,//默认不显示密码
      }
    },
    computed:{
      //电话是否符合规范，返回true或者false
      isRightPhone(){
    ...
    methods:{
      ...
      async login(){
         ...
         if(loginType){//短信
           ...
           result = await reqPwdLogin({ name,code,captcha })
         }
         if(result.code===0){
           //登录成功,将用户数据保存到状态中
           const user = result.data
           this.$store.dispatch('loginIn',user)
           //跳转到个人中心
